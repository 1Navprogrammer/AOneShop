...
namespace AOne
{
    public partial class frmMain : Form
    {
        ...
        AOneShopDBHelper aoneShop = new AOneShopDBHelper();
        ...
        {   // check if customer fileds are empty
            if (tbCustFirstName.Text != "" && tbCustLastName.Text != "" && tbCustAddress.Text != "" && tbCustPhone.Text != "")
            {
                ...
                aoneShop.updateCustomerRecord(custId, firstname, lastname, address, phone);
                refreshCustomerListView(aoneShop.fetchCustomerRecords());
                resetAllInputFields();    
            } else
            {
        ...
        private void BtnAddCust_Click(object sender, EventArgs e)
        {
            if (tbCustFirstName.Text != "" && tbCustLastName.Text != "" && tbCustAddress.Text != "" && tbCustPhone.Text != "")
            {
                ...
                aoneShop.insertCustomerRecord(tbCustFirstName.Text, tbCustLastName.Text, tbCustAddress.Text, tbCustPhone.Text);
                refreshCustomerListView(aoneShop.fetchCustomerRecords()); 
                resetAllInputFields();
            }
            else
        ...
        private void BtnDeleteCust_Click(object sender, EventArgs e)
        {
            if(tbCustID.Text!="")
            {
                ...
                {
                    aoneShop.deleteCustomerRecord(custId);
    ...
    // refresh all list view on form load     
            refreshCustomerListView(aoneShop.fetchCustomerRecords());
            refreshMoviesListView(aoneShop.fetchMovies());
            refreshRentsListView(aoneShop.fetchRentedRecord()); // update list view
        ...
        private void BtnAddMovie_Click(object sender, EventArgs e)
        {
            ...
            if(tbMovieName.Text!="" && tbMovieGenre.Text!="" && tbMovieRating.Text!="" && tbMovieYear.Text!="" && tbMovieCopies.Text!="" && tbMoviePlot.Text!="")
            {
               ...
               // old movie rent =  2 and new movie rent =  5
                ...
                aoneShop.insertMovieRecord(tbMovieRating.Text, tbMovieName.Text, tbMovieYear.Text, rent, tbMoviePlot.Text, tbMovieGenre.Text, copies);
                refreshMoviesListView(aoneShop.fetchMovies());
                resetAllInputFields();
            }else
            {
        ...
        private void BtnUpdateMovie_Click(object sender, EventArgs e)
        {
            ...
            if (tbMovieId.Text != "" && tbMovieName.Text != "" && tbMovieGenre.Text != "" && tbMovieRating.Text != "" && tbMovieYear.Text != "" && tbMovieCopies.Text != "" && tbMoviePlot.Text != "")
            {
                ...
                aoneShop.updateMovieRecord(movieId, rating, title, year, plot, genre, copies);
        ...
        private void BtnDeleteMovie_Click(object sender, EventArgs e)
        {
            if(tbMovieId.Text!="")
            {
                ...
                if(result.ToString()=="Yes")
                {
                    ...
                    aoneShop.deleteMovieRecord(movieId); 
                    refreshMoviesListView(aoneShop.fetchMovies());
                    resetAllInputFields();
                }
            }
        ...
        private void BtnIssueMovie_Click(object sender, EventArgs e)
        {
            ...
            if(tbMovieId.Text !="" && tbCustID.Text!="")
            {
                if(tbMovieCopies.Text!="0")
                {
                    ...
                    aoneShop.insertRentedRecord(movieId, custId, DateTime.Now, copies, 1); // adds rented movie
                    refreshRentsListView(aoneShop.fetchRentedRecord()); // update list view
                    // reset all textboxes
                    resetAllInputFields();
        ...
        private void BtnReturnMovie_Click(object sender, EventArgs e)
        {
            ...
            if(tbMovieId.Text!="" && tbDateRented.Text!="" && tbRMID.Text!="")
            {
                ...
                aoneShop.updateRentedRecord(rmid, movieId, Convert.ToDateTime(date), DateTime.Now);
                
                refreshCustomerListView(aoneShop.fetchCustomerRecords());
                refreshMoviesListView(aoneShop.fetchMovies());
                refreshRentsListView(aoneShop.fetchRentedRecord());
                // clears all inputs
                resetAllInputFields(); 
                resetAllInputFields(); 
                tbRMID.Text = ""; 
        ...
        {
            helperFunctions.fetchBestCustomerRecord();
        }

        private void BtnBestMoie_Click(object sender, EventArgs e)
        {
            helperFunctions.fetchBestMovieRecord();
        }
